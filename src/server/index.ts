...
import { type AuthenticatedAdmin, ShopifyError } from "./libraries/shopify.ts";
import {
  addHeaders,
  generateCspNonce,
  getShopifyHead,
...
app.get("/shopify/install", (c) => {
  ...
  return c.redirect(getInstallUrl(c.env, c.req.raw, shop).toString(), 302);
});

app.get("/shopify/session-token-bounce", (c) => {
  const nonce = generateCspNonce();
  const response = c.html(getSessionTokenBounceHtml(c.env, c.req.raw, nonce));
...
app.get("*", async (c) => {
  ...
  const html = template
    ...
    .replace("<!--head-outlet-->", getShopifyHead(c.env, c.req.raw, nonce))
